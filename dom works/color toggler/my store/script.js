...
function displayAllCategories(clist) {
  let htmlData = ``
  for (let c of clist) {
    htmlData += `
  <div> <button class="btn btn-outline-info" name="${c}" onclick="displayProductsByCategoryName(event)">${c}</button>  </div>
  ...
  let htmlData = ""
    ...
    for (let p of products) {
      htmlData += `
                    ...
                    <div class="card-body">
                      <button class="card-link" name="${p.id}" onclick="getProductById(event)">viewmore</button>
...
function getProductById(event) {
  ...
  let item = service.getProductDetail(id)
  displayProduct(item)
}
...
function displayProductsByCategoryName(event) {
  let c = event.target.name
  let products = service.getProductsbyCategoryname(c)
